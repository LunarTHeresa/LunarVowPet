...
当前行走使用 16 帧透明逐帧小步动画。所有帧统一为相同主体尺寸、脚底基准线与头部高度；窗口每 33 毫秒稳定移动 1 个桌面坐标单位，每移动 3 个单位才推进 1 帧，因此动作和桌面位移共用同一条距离时间轴。左右行走共用同一套帧并自动镜像；停止、暂停、拖拽结束或打开面板时会切换到独立绘制的端庄待机立绘，不会冻结某一张行走帧。到达屏幕边缘时角色会先短暂停稳，再转身继续。
